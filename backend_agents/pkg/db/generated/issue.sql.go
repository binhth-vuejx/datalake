...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, share_token, mcp_config
`
...
type CreateIssueWithOriginRow struct {
...
func (q *Queries) CreateIssueWithOrigin(ctx context.Context, arg CreateIssueWithOriginParams) (CreateIssueWithOriginRow, error) {
	...
	var i CreateIssueWithOriginRow
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
		&i.Title,
...
const generateIssueShareToken = `-- name: GenerateIssueShareToken :one
UPDATE issue
...
func (q *Queries) GenerateIssueShareToken(ctx context.Context, id pgtype.UUID) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, generateIssueShareToken, id)
	var share_token pgtype.UUID
	err := row.Scan(&share_token)
	return share_token, err
}

const getIssue = `-- name: GetIssue :one
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, share_token, mcp_config FROM issue
WHERE id = $1
`
...
const getIssueByShareToken = `-- name: GetIssueByShareToken :one
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, share_token, mcp_config FROM issue
WHERE share_token = $1
`

func (q *Queries) GetIssueByShareToken(ctx context.Context, shareToken pgtype.UUID) (Issue, error) {
	row := q.db.QueryRow(ctx, getIssueByShareToken, shareToken)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
