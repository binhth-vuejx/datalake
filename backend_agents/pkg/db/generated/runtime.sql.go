...
SELECT id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, mcp_config FROM agent_runtime
...
WHERE id = $1
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, mcp_config
`
...
const updateAgentRuntimeMcpConfig = `-- name: UpdateAgentRuntimeMcpConfig :one
UPDATE agent_runtime
...
type UpdateAgentRuntimeMcpConfigParams struct {
	ID        pgtype.UUID `json:"id"`
	McpConfig []byte      `json:"mcp_config"`
}

func (q *Queries) UpdateAgentRuntimeMcpConfig(ctx context.Context, arg UpdateAgentRuntimeMcpConfigParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeMcpConfig, arg.ID, arg.McpConfig)
	var i AgentRuntime
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
DO UPDATE SET
    ...
    last_seen_at = now(),
    updated_at = now()
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, mcp_config, (xmax = 0) AS inserted
`
...
const updateAgentRuntimeMetadata = `-- name: UpdateAgentRuntimeMetadata :exec
UPDATE agent_runtime
...
func (q *Queries) UpdateAgentRuntimeMetadata(ctx context.Context, arg UpdateAgentRuntimeMetadataParams) error {
	_, err := q.db.Exec(ctx, updateAgentRuntimeMetadata, arg.ID, arg.Metadata)
	return err
}
