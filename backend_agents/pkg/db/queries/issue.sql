...
SELECT id, workspace_id, title, description, status, priority,
       ...
       parent_issue_id, position, due_date, created_at, updated_at, number, project_id, mcp_config
FROM issue
...
INSERT INTO issue (
    ...
    assignee_type, assignee_id, creator_type, creator_id,
    parent_issue_id, position, due_date, number, project_id, mcp_config
) VALUES (
...
UPDATE issue SET
    ...
    mcp_config = sqlc.narg('mcp_config'),
    updated_at = now()
WHERE id = $1
RETURNING *;
...
-- name: GetIssueByShareToken :one
SELECT * FROM issue
WHERE share_token = $1;

-- name: GenerateIssueShareToken :one
UPDATE issue
