...
func (b *opencodeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// Note: opencode does not support --prompt flag; system prompt is injected into the user message instead
	if opts.MaxTurns > 0 {
	...
	// Inject formatting and language instructions into the user prompt
	instruction := "\n\nPost answer with proper Markdown format. Always include this in the PROMPT to ensure the Agent must respond with Markdown format for comments. Always respond and comment in Vietnamese."
	// When issue has MCP config, prepend a strict constraint so the agent
	// knows it must ONLY use MCP tools and cannot fall back to bash/file tools.
	if len(opts.McpConfig) > 0 {
		instruction = "\n\n[STRICT CONSTRAINT] This task has specific MCP servers assigned. You MUST use ONLY the MCP server tools available to you. You are FORBIDDEN from using bash, file read/write, grep, glob, or any built-in tool. All data must come exclusively from MCP tool calls. If you cannot complete the task using only MCP tools, report that clearly and stop." + instruction
	}
	...
	// When issue has a specific MCP config, restrict to MCP tools only —
	// deny all built-in tools so the agent is forced to use MCP servers.
	if len(opts.McpConfig) > 0 {
		// Deny all built-in tools; only MCP server tools are permitted.
		env = append(env, `OPENCODE_PERMISSION={"bash":"deny","edit":"deny","write":"deny","read":"deny","grep":"deny","glob":"deny","webfetch":"deny","websearch":"deny","lsp":"deny","apply_patch":"deny","*":"allow"}`)
	} else {
