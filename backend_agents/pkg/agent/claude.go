...
func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// If writing the primary config fails, fall back to the agent config.
	...
	if len(opts.McpConfig) > 0 {
		...
		if err != nil {
			...
			b.cfg.Logger.Warn("failed to write issue mcp_config to temp file, falling back to agent config",
				"error", err)
			// Fall back to agent config if available
			if len(opts.AgentMcpConfig) > 0 {
				path, err = writeMcpConfigToTemp(opts.AgentMcpConfig)
				if err != nil {
					// Log fallback failure but continue without MCP config
					b.cfg.Logger.Warn("failed to write fallback agent mcp_config to temp file, continuing without mcp-config",
						"error", err)
				}
			}
		}
	...
	} else if len(opts.AgentMcpConfig) > 0 {
		// If no issue config but agent config exists, use agent config directly
		path, err := writeMcpConfigToTemp(opts.AgentMcpConfig)
		if err != nil {
			b.cfg.Logger.Warn("failed to write agent mcp_config to temp file, continuing without mcp-config",
				"error", err)
		} else {
			mcpConfigPath = path
			mcpFileCleanup = func() { os.Remove(mcpConfigPath) }
...
var claudeBlockedArgs = map[string]blockedArgMode{
	...
	"--disallowedTools":  blockedWithValue,  // set by daemon when MCP config is present
	"--disallowed-tools": blockedWithValue,  // alias
	"--tools":            blockedWithValue,  // set by daemon when MCP config is present
	"--allowedTools":     blockedWithValue,  // set by daemon when MCP config is present
	"--allowed-tools":    blockedWithValue,  // alias
}
...
func buildClaudeArgs(opts ExecOptions, logger *slog.Logger) []string {
	...
	// When issue has a specific MCP config, restrict built-in tools:
	// - Allow Bash only for multica CLI commands (needed to read issue details and post comments)
	// - Disable all other built-in tools (Edit, Write, Glob, Grep, etc.)
	// Agent must use only the assigned MCP server tools for data access.
	if len(opts.McpConfig) > 0 {
		args = append(args, "--allowedTools", "Bash(multica *)")
	}
...
// If raw is a JSON string (double-encoded), it is unwrapped first.
func writeMcpConfigToTemp(raw json.RawMessage) (string, error) {
	// Unwrap if the value is a JSON string (double-encoded from frontend)
	var strVal string
	if err := json.Unmarshal(raw, &strVal); err == nil {
		// It's a string — use the string content as the actual JSON
		raw = json.RawMessage(strVal)
	}
