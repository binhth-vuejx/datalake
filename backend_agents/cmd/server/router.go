...
func NewRouter(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client) chi.Router {
	...
	// Public issue share link (no auth required)
	r.Get("/api/shared/issues/{token}", h.GetIssueByShareToken)
	r.Get("/api/shared/issues/{token}/timeline", h.GetSharedIssueTimeline)

	// App config (public — CDN domain, PostHog key, etc. — no auth needed)
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/issues", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					r.Post("/share-token", h.GenerateIssueShareToken)
				})
			})
			...
			// Iframes
			r.Route("/api/iframes", func(r chi.Router) {
				r.Get("/", h.ListIframes)
				r.Post("/", h.CreateIframe)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetIframe)
					r.Put("/", h.UpdateIframe)
					r.Delete("/", h.DeleteIframe)
				})
			})
			...
			r.Route("/api/runtimes", func(r chi.Router) {
				...
				r.Route("/{runtimeId}", func(r chi.Router) {
					...
					r.Patch("/mcp-config", h.UpdateRuntimeMcpConfig)
				})
			})
