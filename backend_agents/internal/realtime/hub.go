...
// Message format: {"type":"auth","payload":{"token":"...","workspace_slug":"..."}}
// Returns the token string, workspace slug, or an error description.
func firstMessageAuth(conn *websocket.Conn) (string, string, string) {
	...
	return msg.Payload.Token, msg.Payload.WorkspaceSlug, ""
}
...
func HandleWebSocket(hub *Hub, mc MembershipChecker, pr PATResolver, resolveSlug SlugResolver, w http.ResponseWriter, r *http.Request) {
	...
	// Get workspace ID from query parameters, header, or user context
	...
	// Try to get workspace_slug from X-Workspace-Slug header (set by client)
	if workspaceID == "" {
		if slug := r.Header.Get("X-Workspace-Slug"); slug != "" && resolveSlug != nil {
			resolved, err := resolveSlug(r.Context(), slug)
			if err != nil {
				http.Error(w, `{"error":"workspace not found"}`, http.StatusNotFound)
				return
	...
	// Try to get workspace_slug from last_workspace_slug cookie (web clients)
	if workspaceID == "" && resolveSlug != nil {
		if cookie, err := r.Cookie("last_workspace_slug"); err == nil && cookie.Value != "" {
			if resolved, err := resolveSlug(r.Context(), cookie.Value); err == nil {
				workspaceID = resolved
			}
		}
	}
	
	// If still no workspace and user is authenticated via cookie, that's OK
	// (user will authenticate via first message with workspace info)
	...
	if userID == "" {
		tokenStr, slugFromMsg, errMsg := firstMessageAuth(conn)
		if errMsg != "" {
		...
		// Use workspace_slug from first message if not already set
		if workspaceID == "" && slugFromMsg != "" && resolveSlug != nil {
			resolved, err := resolveSlug(r.Context(), slugFromMsg)
			if err != nil {
				conn.WriteMessage(websocket.TextMessage, []byte(`{"error":"workspace not found"}`))
				conn.Close()
				return
			}
			workspaceID = resolved
		...
		// Cookie auth succeeded — send ack, workspace already resolved from cookie/header
		conn.WriteMessage(websocket.TextMessage, []byte(`{"type":"auth_ack"}`))
	}

	// If still no workspace, reject
	if workspaceID == "" {
		conn.WriteMessage(websocket.TextMessage, []byte(`{"error":"workspace_id or workspace_slug required"}`))
		conn.Close()
		return
	}
