...
func (h *Handler) ListPins(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range pins {
		...
		case "iframe":
			iframe, err := h.Queries.GetIframe(r.Context(), p.ItemID)
			if err != nil {
				continue // Skip deleted items
			}
			pr.Title = iframe.Title
			pr.Icon = textToPtr(iframe.Icon)
		}
		resp = append(resp, pr)
	}
...
func (h *Handler) CreatePin(w http.ResponseWriter, r *http.Request) {
	...
	if req.ItemType != "issue" && req.ItemType != "project" && req.ItemType != "iframe" {
		writeError(w, http.StatusBadRequest, "item_type must be 'issue', 'project', or 'iframe'")
		return
	}
	if req.ItemID == "" {
