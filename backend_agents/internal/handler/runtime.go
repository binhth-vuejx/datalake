...
func runtimeToResponse(rt db.AgentRuntime) AgentRuntimeResponse {
	...
	if len(rt.McpConfig) > 0 {
		s := string(rt.McpConfig)
		mcpConfig = &s
	}
...
// UpdateRuntimeMcpConfig updates the mcp_config for a runtime.
// Only the runtime owner or workspace admin/owner can update it.
// Supported providers: claude, codex, opencode, gemini.
func (h *Handler) UpdateRuntimeMcpConfig(w http.ResponseWriter, r *http.Request) {
	runtimeID := chi.URLParam(r, "runtimeId")
	...
	// Permission: owner/admin can update any runtime; members can only update their own.
	userID := uuidToString(member.UserID)
	isAdmin := roleAllowed(member.Role, "owner", "admin")
	isOwner := rt.OwnerID.Valid && uuidToString(rt.OwnerID) == userID
	if !isAdmin && !isOwner {
	...
	// Only allow MCP config for supported providers.
	...
	if !supportedProviders[rt.Provider] {
		writeError(w, http.StatusBadRequest, "mcp_config is only supported for claude, codex, opencode, and gemini runtimes")
		return
	}
	...
	var rawMcp json.RawMessage
	if req.McpConfig != nil && *req.McpConfig != "" {
		rawMcp = json.RawMessage(*req.McpConfig)
		if err := validateMcpConfig(rawMcp); err != nil {
			writeError(w, http.StatusBadRequest, err.Error())
			return
		}
	}

	updated, err := h.Queries.UpdateAgentRuntimeMcpConfig(r.Context(), db.UpdateAgentRuntimeMcpConfigParams{
		ID:        rt.ID,
		McpConfig: rawMcp,
	})
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to update mcp_config")
		return
	}

	// Queue a write to the runtime's local config file via next heartbeat.
	h.McpWriteStore.Queue(runtimeID, string(rawMcp))
