...
func (h *Handler) DaemonRegister(w http.ResponseWriter, r *http.Request) {
	...
	for _, runtime := range req.Runtimes {
		...
		if len(runtime.McpServers) > 0 {
			metadataMap["mcp_servers"] = runtime.McpServers
		}
...
func (h *Handler) DaemonHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	rt, err := h.Queries.GetAgentRuntime(r.Context(), parseUUID(req.RuntimeID))
	if err != nil {
		writeError(w, http.StatusInternalServerError, "heartbeat failed")
		return
	}

	// If daemon sent mcp_servers, merge into metadata.
	if len(req.McpServers) > 0 {
		var existing map[string]any
		if len(rt.Metadata) > 0 {
			json.Unmarshal(rt.Metadata, &existing)
		}
		if existing == nil {
			existing = map[string]any{}
		}
		existing["mcp_servers"] = req.McpServers
		if newMeta, err := json.Marshal(existing); err == nil {
			h.Queries.UpdateAgentRuntimeMetadata(r.Context(), db.UpdateAgentRuntimeMetadataParams{
				ID:       rt.ID,
				Metadata: newMeta,
			})
		}
	...
	// Check for pending MCP config write requests for this runtime.
	if mcpConfig, ok := h.McpWriteStore.PopPending(req.RuntimeID); ok {
		resp["pending_mcp_config_write"] = map[string]string{"mcp_config": mcpConfig}
	}
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	if task.IssueID.Valid {
		if issue, err := h.Queries.GetIssue(r.Context(), task.IssueID); err == nil {
			...
			// Include Issue's mcp_config in the Task payload if present
			if len(issue.McpConfig) > 0 {
				resp.IssueMcpConfig = json.RawMessage(issue.McpConfig)
			}
		}
