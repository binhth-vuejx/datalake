...
func issueToResponse(i db.Issue, issuePrefix string) IssueResponse {
	...
	return IssueResponse{
		...
		McpConfig:     json.RawMessage(i.McpConfig),
		ShareToken:    uuidToPtr(i.ShareToken),
	}
}
...
// validateMcpConfig validates that raw is valid JSON and contains a top-level "mcpServers" key.
// Returns nil if raw is nil/empty (absent field is valid).
func validateMcpConfig(raw json.RawMessage) error {
	if len(raw) == 0 {
		return nil
	}
	// Allow explicit null (clears the field)
	if string(raw) == "null" {
		return nil
	}

	// First, try to unmarshal as a string (mcp_config is sent as a JSON string from frontend)
	var configStr string
	if err := json.Unmarshal(raw, &configStr); err == nil {
		// It's a string, so unmarshal the string value as JSON
		raw = []byte(configStr)
	}
	// If it's not a string, raw remains unchanged and we'll try to unmarshal it as an object

	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
		return fmt.Errorf("mcp_config must be valid JSON")
	}
	mcpServersRaw, ok := obj["mcpServers"]
	if !ok {
		return fmt.Errorf("mcp_config must contain a top-level 'mcpServers' object")
	}
	// Verify mcpServers is not null
	if string(mcpServersRaw) == "null" {
		return fmt.Errorf("mcp_config must contain a top-level 'mcpServers' object")
	}
	// Verify mcpServers is an object
	var mcpServersObj map[string]interface{}
	if err := json.Unmarshal(mcpServersRaw, &mcpServersObj); err != nil {
		return fmt.Errorf("mcp_config must contain a top-level 'mcpServers' object")
	}
	return nil
}
...
type UpdateIssueRequest struct {
	...
	ParentIssueID *string          `json:"parent_issue_id"`
	ProjectID     *string          `json:"project_id"`
	McpConfig     *json.RawMessage `json:"mcp_config"` // pointer to distinguish absent vs null
}
...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["mcp_config"]; ok {
		if req.McpConfig != nil {
			...
			params.McpConfig = json.RawMessage(nil) // explicit null = clear config
		}
	}
...
// GetIssueByShareToken returns an issue by its share token (public, no auth required)
func (h *Handler) GetIssueByShareToken(w http.ResponseWriter, r *http.Request) {
	tokenStr := chi.URLParam(r, "token")
	if tokenStr == "" {
		writeError(w, http.StatusBadRequest, "share token is required")
		return
	}

	token := parseUUID(tokenStr)
	if !token.Valid {
		writeError(w, http.StatusBadRequest, "invalid share token format")
		return
	}

	issue, err := h.Queries.GetIssueByShareToken(r.Context(), token)
	if err != nil {
		writeError(w, http.StatusNotFound, "issue not found or invalid share token")
		return
	}
...
// GenerateIssueShareToken generates a share token for an issue (requires auth)
func (h *Handler) GenerateIssueShareToken(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	issue, ok := h.loadIssueForUser(w, r, id)
	if !ok {
		return
	...
	token, err := h.Queries.GenerateIssueShareToken(r.Context(), issue.ID)
	if err != nil {
		// If token already exists, fetch the existing issue to get the token
		if issue.ShareToken.Valid {
			token = issue.ShareToken
		} else {
			slog.Warn("generate share token failed", append(logger.RequestAttrs(r), "error", err, "issue_id", id)...)
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"share_token": uuidToString(token),
	})
}
