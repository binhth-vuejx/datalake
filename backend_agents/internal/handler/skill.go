...
func skillToResponse(s db.Skill) SkillResponse {
	return skillToResponseWithStrict(s, false)
}

func skillToResponseWithStrict(s db.Skill, strict bool) SkillResponse {
	var config any
	if s.Config != nil {
...
func (h *Handler) ListAgentSkills(w http.ResponseWriter, r *http.Request) {
	...
	resp := make([]SkillResponse, len(skills))
	for i, s := range skills {
		// Convert ListAgentSkillsRow to Skill
		skill := db.Skill{
			ID:          s.ID,
