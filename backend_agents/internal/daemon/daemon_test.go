...
// TestRunTaskIssueMcpConfigTakesPrecedence verifies that when an Issue has
// mcp_config, the daemon uses it instead of the Agent's mcp_config.
// Validates: Requirement 2.1, 2.5
func TestRunTaskIssueMcpConfigTakesPrecedence(t *testing.T) {
	t.Parallel()

	issueMcpConfig := json.RawMessage(`{"mcpServers":{"github":{"command":"npx"}}}`)
	agentMcpConfig := json.RawMessage(`{"mcpServers":{"slack":{"command":"npx"}}}`)

	task := Task{
		ID:             "task-1",
		WorkspaceID:    "ws-1",
		IssueID:        "issue-1",
		IssueMcpConfig: issueMcpConfig,
		Agent: &AgentData{
			ID:        "agent-1",
			Name:      "Test Agent",
	...
	// Simulate the priority logic from runTask
	...
	if !bytes.Equal(mcpConfig, issueMcpConfig) {
		t.Fatalf("expected Issue mcp_config to take precedence, got %s", string(mcpConfig))
	}
}

// TestRunTaskFallbackToAgentMcpConfig verifies that when an Issue has no
// mcp_config, the daemon falls back to the Agent's mcp_config.
// Validates: Requirement 2.2
func TestRunTaskFallbackToAgentMcpConfig(t *testing.T) {
	t.Parallel()
	...
	if !bytes.Equal(mcpConfig, agentMcpConfig) {
		t.Fatalf("expected Agent mcp_config as fallback, got %s", string(mcpConfig))
	}
}

// TestRunTaskBothMcpConfigNull verifies that when both Issue and Agent have
// no mcp_config, no McpConfig is passed to ExecOptions.
// Validates: Requirement 2.3
func TestRunTaskBothMcpConfigNull(t *testing.T) {
	t.Parallel()
	...
	if len(mcpConfig) > 0 {
		t.Fatalf("expected no McpConfig when both are null, got %s", string(mcpConfig))
	}
}

// TestRunTaskNoAgentData verifies that when there's no Agent data at all,
// no McpConfig is passed to ExecOptions.
// Validates: Requirement 2.3
func TestRunTaskNoAgentData(t *testing.T) {
	t.Parallel()
	...
	if len(mcpConfig) > 0 {
		t.Fatalf("expected no McpConfig when Agent is nil, got %s", string(mcpConfig))
	}
}

// TestRunTaskIssueMcpConfigCompletelyReplaces verifies that Issue mcp_config
// completely replaces Agent mcp_config (no merging).
// Validates: Requirement 2.5
func TestRunTaskIssueMcpConfigCompletelyReplaces(t *testing.T) {
	t.Parallel()

	// Agent has both github and slack
	agentMcpConfig := json.RawMessage(`{"mcpServers":{"github":{"command":"npx"},"slack":{"command":"npx"}}}`)
	// Issue only has github
	issueMcpConfig := json.RawMessage(`{"mcpServers":{"github":{"command":"npx"}}}`)
	...
	// Verify that we got exactly the Issue config, not a merge
	if !bytes.Equal(mcpConfig, issueMcpConfig) {
		t.Fatalf("expected Issue mcp_config to completely replace Agent config, got %s", string(mcpConfig))
	}

	// Verify it's not the agent config
	if bytes.Equal(mcpConfig, agentMcpConfig) {
		t.Fatal("Issue config should not equal Agent config (they should be different)")
	}
}

// TestRunTaskMcpConfigPassedToExecOptions verifies that the selected mcp_config
// is correctly passed to ExecOptions.McpConfig.
// Validates: Requirement 2.1
func TestRunTaskMcpConfigPassedToExecOptions(t *testing.T) {
	t.Parallel()
	...
	// Simulate the priority logic and ExecOptions construction
	var mcpConfig json.RawMessage
	if len(task.IssueMcpConfig) > 0 {
		mcpConfig = task.IssueMcpConfig
	} else if task.Agent != nil {
	...
	if !bytes.Equal(execOpts.McpConfig, issueMcpConfig) {
		t.Fatalf("expected ExecOptions.McpConfig to be set to Issue config, got %s", string(execOpts.McpConfig))
	}
}

// TestRunTaskAgentMcpConfigFallback verifies that AgentMcpConfig is set as
// a fallback in ExecOptions for error handling.
// Validates: Requirement 6.3 (fallback handling)
func TestRunTaskAgentMcpConfigFallback(t *testing.T) {
	t.Parallel()
	...
	// Simulate the logic from runTask
	var mcpConfig json.RawMessage
	var agentMcpConfigFallback json.RawMessage
	if task.Agent != nil {
		agentMcpConfigFallback = task.Agent.McpConfig
	}
	if len(task.IssueMcpConfig) > 0 {
		mcpConfig = task.IssueMcpConfig
	} else if task.Agent != nil {
	...
	execOpts := agent.ExecOptions{
		McpConfig:      mcpConfig,
		AgentMcpConfig: agentMcpConfigFallback,
	}

	if !bytes.Equal(execOpts.AgentMcpConfig, agentMcpConfig) {
		t.Fatalf("expected AgentMcpConfig fallback to be set, got %s", string(execOpts.AgentMcpConfig))
	}
}

// TestRunTaskIssueMcpConfigWithAgentFallback verifies that when Issue has
// mcp_config, AgentMcpConfig is still set as fallback for error handling.
// Validates: Requirement 6.3
func TestRunTaskIssueMcpConfigWithAgentFallback(t *testing.T) {
	t.Parallel()
	...
	// Verify Issue config is used
	if !bytes.Equal(execOpts.McpConfig, issueMcpConfig) {
		t.Fatalf("expected McpConfig to be Issue config, got %s", string(execOpts.McpConfig))
	}

	// Verify Agent config is still available as fallback
	if !bytes.Equal(execOpts.AgentMcpConfig, agentMcpConfig) {
		t.Fatalf("expected AgentMcpConfig fallback to be set, got %s", string(execOpts.AgentMcpConfig))
	}
}

// TestRunTaskEmptyIssueMcpConfigTreatsAsNull verifies that an empty
// IssueMcpConfig is treated as null (falls back to Agent config).
// Validates: Requirement 2.2
func TestRunTaskEmptyIssueMcpConfigTreatsAsNull(t *testing.T) {
	t.Parallel()
	...
	task := Task{
		...
		WorkspaceID:    "ws-1",
		IssueID:        "issue-1",
		IssueMcpConfig: json.RawMessage{}, // empty, not nil
		Agent: &AgentData{
			ID:        "agent-1",
			Name:      "Test Agent",
			McpConfig: agentMcpConfig,
	...
	if !bytes.Equal(mcpConfig, agentMcpConfig) {
		t.Fatalf("expected empty IssueMcpConfig to fall back to Agent config, got %s", string(mcpConfig))
	}
}
