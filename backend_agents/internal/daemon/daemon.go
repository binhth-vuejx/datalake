...
// appendFormatSection appends the mandatory format section to agent output
// Currently disabled to keep agent response intact
func appendFormatSection(output string) string {
	// Return original output without appending format section
	return output
}
...
func (d *Daemon) registerRuntimesForWorkspace(ctx context.Context, workspaceID string) (*RegisterResponse, error) {
	...
	for name, entry := range d.cfg.Agents {
		...
		// Read MCP servers from the runtime's native config file.
		if mcpServers, err := ReadRuntimeMcpServers(name); err == nil && len(mcpServers) > 0 {
			rt["mcp_servers"] = mcpServers
		}
	...
	for {
		...
		case <-ticker.C:
			for _, rid := range d.allRuntimeIDs() {
				// Read current MCP servers from the runtime's config file.
				var mcpServers []McpServerInfo
				if rt := d.findRuntime(rid); rt != nil {
					mcpServers, _ = ReadRuntimeMcpServers(rt.Provider)
				}

				resp, err := d.client.SendHeartbeat(ctx, rid, mcpServers)
				if err != nil {
				...
				// Handle pending MCP config write requests.
				if resp.PendingMcpConfigWrite != nil {
					rt := d.findRuntime(rid)
					if rt != nil {
						go d.handleMcpConfigWrite(ctx, *rt, resp.PendingMcpConfigWrite.McpConfig)
					}
				}
			}
		}
...
func hasSkillsForTask(task Task) bool {
...
// verifySkillCompliance checks if the task result matches the assigned skill requirements
func (d *Daemon) verifySkillCompliance(ctx context.Context, task Task, result agent.Result, taskLog *slog.Logger) (bool, string) {
	taskLog.Info("skill compliance check started", "has_agent", task.Agent != nil, "skills_count", len(task.Agent.Skills))

	if !hasSkillsForTask(task) {
		taskLog.Info("no skills to verify")
		return true, "" // No skills to verify
	}

	// Check if any skill is in strict mode
	hasStrictMode := false
	for _, skill := range task.Agent.Skills {
		taskLog.Info("checking skill", "name", skill.Name, "strict", skill.Strict)
		if skill.Strict {
			hasStrictMode = true
			break
		}
	}
	taskLog.Info("strict mode", "enabled", hasStrictMode)

	// Basic verification: check if result output is non-empty
	if result.Output == "" {
		taskLog.Warn("result output is empty")
		return false, "Result output is empty"
	}

	// Checkpoint validation: parse output for skill checkpoint message
	// Expected patterns: "=== SKILL CONFIRMATION ===" with checklist items
	outputLower := strings.ToLower(result.Output)
	hasCheckpoint := false
	confirmedSkills := make(map[string]bool)

	// Check for skill confirmation section (case insensitive)
	confirmationPattern := "skill confirmation"
	if strings.Contains(outputLower, confirmationPattern) {
		hasCheckpoint = true
		// Extract skill name and checklist verification
		lines := strings.Split(result.Output, "\n")
		inConfirmationSection := false
		currentSkill := ""

		for _, line := range lines {
			lineLower := strings.ToLower(line)
			if strings.Contains(lineLower, confirmationPattern) {
				inConfirmationSection = true
				continue
			}

			if inConfirmationSection {
				if strings.Contains(lineLower, "skill:") {
					parts := strings.SplitN(line, ":", 2)
					if len(parts) == 2 {
						currentSkill = strings.TrimSpace(parts[1])
					}
				}

				// Check for checklist items with [x] (case insensitive)
				if strings.Contains(line, "[x]") {
					if currentSkill != "" {
						confirmedSkills[currentSkill] = true
					}
				}

				// End of confirmation section - check for new section markers
				if strings.HasPrefix(line, "===") && !strings.Contains(lineLower, confirmationPattern) {
					inConfirmationSection = false
				}
				// Also end if we see another major section
				if strings.HasPrefix(line, "#") && inConfirmationSection {
					inConfirmationSection = false
				}
			}
		}
	...
	// Fallback: check for simple skill loading messages if confirmation section not found
	if !hasCheckpoint {
		// Check for new format section with "Skills sử dụng:" field
		if strings.Contains(result.Output, "Skills sử dụng:") {
			hasCheckpoint = true
			// Extract skill names from "Skills sử dụng:" field
			lines := strings.Split(result.Output, "\n")
			for _, line := range lines {
				if strings.Contains(line, "Skills sử dụng:") {
					// Mark all assigned skills as confirmed if field is present
					for _, skill := range task.Agent.Skills {
						confirmedSkills[skill.Name] = true
					}
					break
				}
			}
		} else {
			// Original fallback: check for simple skill loading messages
			skillPhrases := []string{
				"skill loaded:",
				"loaded skill:",
				"verified skill:",
			...
			for _, skill := range task.Agent.Skills {
				...
				for _, phrase := range skillPhrases {
					...
					if strings.Contains(outputLower, phraseLower+skillNameLower) {
						hasCheckpoint = true
						confirmedSkills[skill.Name] = true
						break
					}
				}
				if confirmedSkills[skill.Name] {
					break
				}
			}
		}
	...
	// In strict mode, checkpoint is mandatory. In non-strict mode, it's optional but logged.
	if hasStrictMode {
		if len(task.Agent.Skills) > 0 && !hasCheckpoint {
			skillNames := make([]string, 0, len(task.Agent.Skills))
			for _, s := range task.Agent.Skills {
				if s.Strict {
					skillNames = append(skillNames, s.Name)
				}
			}
			return false, fmt.Sprintf("STRICT MODE: No skill checkpoint found in output. Expected to see 'Skill loaded: %s' or similar checkpoint message. Agent must explicitly confirm skill loading before proceeding.", strings.Join(skillNames, " or "))
		}

		// Check if all strict skills have checkpoints
		for _, skill := range task.Agent.Skills {
			if skill.Strict && !confirmedSkills[skill.Name] {
				return false, fmt.Sprintf("STRICT MODE: Skill '%s' is marked as strict but no checkpoint found. Agent must confirm loading of all strict skills.", skill.Name)
			}
		}
	} else {
		// Non-strict mode: log warning but don't fail
		if len(task.Agent.Skills) > 0 && !hasCheckpoint {
			taskLog.Info("skill compliance: no checkpoint found (non-strict mode, proceeding)", "skills", len(task.Agent.Skills))
		}
	}

	taskLog.Info("skill compliance verified", "skills", len(confirmedSkills), "strict_mode", hasStrictMode)
	return true, ""
}
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	...
	// Priority: Issue.mcp_config > Agent.mcp_config
	// If task.IssueMcpConfig is non-empty, use it; otherwise fall back to agent config
	if len(task.IssueMcpConfig) > 0 {
		mcpConfig = task.IssueMcpConfig
	...
	// When agent has strict skills, inject skill content into SystemPrompt
	// so the agent receives it directly — no file reading needed.
	if task.Agent != nil {
		strictSkillPrompt := buildStrictSkillSystemPrompt(task.Agent.Skills)
		if strictSkillPrompt != "" {
			if execOpts.SystemPrompt != "" {
				execOpts.SystemPrompt = execOpts.SystemPrompt + "\n\n" + strictSkillPrompt
			} else {
				execOpts.SystemPrompt = strictSkillPrompt
			}
		}
	}
	...
	// Skill enforcement: verify result matches skill requirements and retry if needed
	taskLog.Info("skill enforcement check", "has_agent", task.Agent != nil, "agent_skills_count", len(task.Agent.Skills))
	if task.Agent != nil {
		for i, skill := range task.Agent.Skills {
			taskLog.Info("agent skill", "index", i, "name", skill.Name, "strict", skill.Strict)
		}
	}

	if hasSkillsForTask(task) && result.Status == "completed" {
		taskLog.Info("initiating skill compliance verification")
		verified, reason := d.verifySkillCompliance(ctx, task, result, taskLog)
		if !verified {
			taskLog.Warn("skill compliance verification failed, retrying task", "reason", reason)

			// Retry up to 2 times for skill compliance
			maxRetries := 2
			for i := 0; i < maxRetries; i++ {
				taskLog.Info("skill compliance retry attempt", "attempt", i+1, "max", maxRetries)

				// Re-execute with additional enforcement prompt
				enforcedPrompt := prompt + "\n\nPREVIOUS ATTEMPT FAILED SKILL COMPLIANCE.\n" +
					"Reason: " + reason + "\n" +
					"You MUST strictly follow the assigned skill requirements this time.\n" +
					"Review the skill content again and ensure your output matches exactly.\n"

				retryResult, retryTools, retryErr := d.executeAndDrain(ctx, backend, enforcedPrompt, execOpts, taskLog, task.ID)
				if retryErr != nil {
					taskLog.Error("skill compliance retry failed", "attempt", i+1, "error", retryErr)
					continue
				}

				retryVerified, retryReason := d.verifySkillCompliance(ctx, task, retryResult, taskLog)
				if retryVerified {
					taskLog.Info("skill compliance verified after retry", "attempt", i+1)
					result = retryResult
					tools = retryTools
					result.Usage = mergeUsage(result.Usage, retryResult.Usage)
					break
				} else {
					taskLog.Warn("skill compliance still failed after retry", "attempt", i+1, "reason", retryReason)
					result = retryResult
					tools = retryTools
					result.Usage = mergeUsage(result.Usage, retryResult.Usage)
				}
			...
			// If still not verified after retries, mark as blocked with explanation
			finalVerified, finalReason := d.verifySkillCompliance(ctx, task, result, taskLog)
			if !finalVerified {
				taskLog.Error("skill compliance failed after all retries", "reason", finalReason)
				return TaskResult{
					Status:    "blocked",
					Comment:   fmt.Sprintf("Skill compliance verification failed: %s\n\nThe task result does not match the assigned skill requirements. Please review the skill and try again.", finalReason),
	...
	case "completed":
		...
		// Append mandatory format section to output
		commentWithFormat := appendFormatSection(result.Output)

		return TaskResult{
			Status:    "completed",
			Comment:   commentWithFormat,
			SessionID: result.SessionID,
			WorkDir:   env.WorkDir,
			EnvRoot:   env.RootDir,
			Usage:     usageEntries,
