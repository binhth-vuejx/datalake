...
// Preserves original case so Claude's Skill tool can find it by name.
func sanitizeSkillName(name string) string {
	s := strings.TrimSpace(name)
	// Replace spaces and special chars with hyphens, preserve case
...
// Removes any existing skill directories not in the current assignment to prevent
// agents from reading skills assigned to other agents.
func writeSkillFiles(skillsDir string, skills []SkillContextForEnv) error {
	...
	for _, skill := range skills {
		expectedSkills[sanitizeSkillName(skill.Name)] = true
	}

	// Remove any skill directories not in current assignment
	entries, err := os.ReadDir(skillsDir)
	if err == nil {
		for _, entry := range entries {
			if entry.IsDir() {
				if !expectedSkills[entry.Name()] {
					// Remove old skill directory not assigned to this agent
					oldPath := filepath.Join(skillsDir, entry.Name())
