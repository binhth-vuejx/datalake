...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue comment add <issue-id> --content-stdin [--parent <comment-id>]` — Post a comment by piping content via stdin (preserves newlines and formatting)\n")
	b.WriteString("  - Usage: `cat > /tmp/reply.md << 'EOF' ... EOF && multica issue comment add <id> --content-stdin < /tmp/reply.md`\n")
	b.WriteString("  - For simple one-liners only: `multica issue comment add <issue-id> --content \"short text\" [--parent <comment-id>]`\n")
	...
	if ctx.ChatSessionID != "" {
		...
		fmt.Fprintf(&b, "5. **Post your final results as a comment — this step is mandatory.** Write your response to a temp file and pipe it to preserve newlines:\n")
		fmt.Fprintf(&b, "   ```\n   cat > /tmp/reply.md << 'REPLY_EOF'\n   <your full response here>\n   REPLY_EOF\n   multica issue comment add %s --content-stdin < /tmp/reply.md\n   ```\n", ctx.IssueID)
		b.WriteString("   Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n")
	...
	if len(ctx.AgentSkills) > 0 {
		...
		switch provider {
		case "claude":
			b.WriteString("You have the following skills installed (discovered automatically from .claude/skills/):\n\n")
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi":
