...
import { Plus, FileText, Trash2, Info, Shield, ShieldCheck } from "lucide-react";
...
export function SkillsTab({
  ...
  // Load strict mode from server data - check if any skill has strict=true
  const hasStrictSkill = agent.skills.length > 0 && agent.skills.some(s => s.strict === true);
  const [enableStrict, setEnableStrict] = useState(hasStrictSkill);
  ...
  const handleAdd = async (skillId: string) => {
    ...
    try {
      ...
      await api.setAgentSkills(agent.id, { skill_ids: newIds, strict: enableStrict });
  ...
  const handleToggleStrict = async () => {
    setSaving(true);
    try {
      const newStrict = !enableStrict;
      setEnableStrict(newStrict);
      ...
      toast.error(e instanceof Error ? e.message : "Failed to update strict mode");
      setEnableStrict(!enableStrict); // Revert on error
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {enableStrict
                  ? "Agent must confirm skill loading with checkpoint message"
                  : "Skill usage is recommended but not enforced"}
              </div>
            </div>
          </div>
          <Switch
            checked={enableStrict}
