...
import { StatusIcon, PriorityIcon, StatusPicker, PriorityPicker, DueDatePicker, AssigneePicker, canAssignAgent, McpPicker } from ".";
...
import { issueListOptions, issueDetailOptions, childIssuesOptions, issueUsageOptions, issueByShareTokenOptions, sharedIssueTimelineOptions } from "@multica/core/issues/queries";
import { memberListOptions, agentListOptions } from "@multica/core/workspace/queries";
import { runtimeListOptions, buildMcpConfigFromRuntime } from "@multica/core/runtimes";
...
export function IssueDetail({ issueId, shareToken, onDelete, defaultSidebarOpen = true, layoutId = "multica_issue_detail_layout", highlightCommentId }: IssueDetailProps) {
  const id = issueId;
  const router = useNavigation();
  const user = useAuthStore((s) => s.user);
  const userId = useAuthStore((s) => s.user?.id);
  const workspace = useCurrentWorkspace();
  const paths = useWorkspacePathsSafe();
  const isSharedView = !!shareToken;

  // Issue navigation — read from TQ list cache
  // useWorkspaceId() throws when there's no workspace context (e.g. shared issue page),
  // so fall back to workspace from useCurrentWorkspace() which returns null safely.
  ...
  const { data: issue = null, isLoading: issueLoading } = useQuery(
    (isSharedView && shareToken ? issueByShareTokenOptions(shareToken) : issueDetailOptions(wsId, id ?? "")) as any
  ) as { data: Issue | null; isLoading: boolean };

  // Record recent visit
  const recordVisit = useRecentIssuesStore((s) => s.recordVisit);
  useEffect(() => {
    if (issue && !isSharedView) {
      recordVisit(issue.id);
    }
  }, [issue?.id, isSharedView]); // eslint-disable-line react-hooks/exhaustive-deps

  // Custom hooks — encapsulate timeline, reactions, subscribers
  const {
    timeline: authTimeline, submitComment, submitReply,
    editComment, deleteComment, toggleReaction: handleToggleReaction,
  } = useIssueTimeline(isSharedView ? "" : (issue?.id ?? ""), user?.id);

  // In shared view, fetch timeline via the public endpoint (no auth required)
  const { data: sharedTimeline = [] } = useQuery({
    ...sharedIssueTimelineOptions(shareToken ?? ""),
    enabled: isSharedView && !!shareToken,
  });

  const timeline = isSharedView ? sharedTimeline : authTimeline;

  const {
    reactions: issueReactions,
    toggleReaction: handleToggleIssueReaction,
  } = useIssueReactions(issue?.id ?? "", user?.id);

  const {
    subscribers, isSubscribed, toggleSubscribe: handleToggleSubscribe, toggleSubscriber,
  } = useIssueSubscribers(issue?.id ?? "", user?.id);

  // Token usage
  const { data: usage } = useQuery(issueUsageOptions(issue?.id ?? ""));

  // Pinned state
  const { data: pinnedItems = [] } = useQuery({
    ...pinListOptions(wsId, userId ?? ""),
    enabled: !!userId && !isSharedView,
  });
  const isPinned = pinnedItems.some((p) => p.item_type === "issue" && p.item_id === issue?.id);
  ...
  const { data: parentIssue = null } = useQuery({
    ...
    enabled: !!parentIssueId && !isSharedView,
    initialData: () => allIssues.find((i) => i.id === parentIssueId),
  });
  const { data: childIssues = [] } = useQuery({
    ...childIssuesOptions(wsId, issue?.id ?? ""),
    enabled: !!issue && !isSharedView,
  });
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {propertiesOpen && <div className="space-y-0.5 pl-2">
          ...
          <PropRow label="MCP tools">
            <McpPicker
              mcpConfig={issue.mcp_config}
              agentMcpConfig={(() => {
                ...
                return buildMcpConfigFromRuntime(runtime);
              })()}
      ...
      <div className="flex h-full flex-col">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-auto">
                ...
                {/* Copy share link - only show in authenticated view */}
                {!isSharedView && (
                  <DropdownMenuItem onClick={async () => {
                    if (!issue?.id) return;
                    try {
                      const { share_token } = await api.generateIssueShareToken(issue.id);
                      const shareUrl = `${window.location.origin}/shared/issues/${share_token}`;
            ...
            <IssuePickerDialog
              ...
              excludeIds={[issue?.id ?? "", ...(parentIssueId ? [parentIssueId] : []), ...childIssues.map((c) => c.id)]}
              onSelect={(selected) => {
                if (!issue) return;
                updateIssueMutation.mutate(
        ...
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          ...
          <div>
            ...
            {!isSharedView && issue?.id && <AgentLiveCard key={issue.id} issueId={issue.id} />}

            {/* Agent execution history */}
            {!isSharedView && issue?.id && (
              <div className="mt-3">
                <TaskRunHistory key={issue.id} issueId={issue.id} />
              </div>
            )}
            ...
            {/* Bottom comment input — hidden in shared view */}
            {!isSharedView && (
              <div className="mt-4">
