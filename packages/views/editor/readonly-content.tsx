...
import { EChartsChart } from "@multica/ui/components/ui/echarts-chart";
import { useWorkspacePathsSafe, useWorkspaceSlug } from "@multica/core/paths";
...
// ---------------------------------------------------------------------------
// ECharts — parse JSON option string, return null on failure
// ---------------------------------------------------------------------------

function parseEChartsOption(code: string): EChartsOption | null {
  try {
    return JSON.parse(code) as EChartsOption;
  } catch {
    return null;
  }
}
...
const components: Partial<Components> = {
  ...
  code: ({ className, children, node, ...props }) => {
    ...
    // ECharts block — attempt JSON parse and render chart
    if (className === "language-echarts") {
      const parsed = parseEChartsOption(code);
      if (parsed !== null) {
        const mergedOption: EChartsOption = { ...parsed, backgroundColor: "transparent" };
        return <EChartsChart option={mergedOption} className="echarts-chart-wrapper" />;
      }
      // Invalid JSON — fall through to lowlight fallback below
    }
...
export function ReadonlyContent({ content, className }: ReadonlyContentProps) {
  const processed = useMemo(() => {
    let result = preprocessMarkdown(content);
    // Convert single newlines to markdown hard line breaks (2 trailing spaces + \n)
    // so agent output with \n renders as actual line breaks without breaking inline bold/italic.
    // Only convert single \n — double \n (paragraph breaks) are left untouched.
