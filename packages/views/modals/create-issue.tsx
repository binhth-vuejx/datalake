...
import { StatusIcon, StatusPicker, PriorityPicker, AssigneePicker, DueDatePicker, McpPicker, isSelectionValidForAgent } from "../issues/components";
...
export function CreateIssueModal({ onClose, data }: { onClose: () => void; data?: Record<string, unknown> | null }) {
  ...
  const [previousAssigneeId, setPreviousAssigneeId] = useState<string | undefined>(draft.assigneeId);

  // Query to get agent's mcp_config from runtime
  const wsId = useCurrentWorkspace()?.id;
  const { data: agentsData } = useQuery(agentListOptions(wsId || ""));
  const { data: runtimesData } = useQuery(runtimeListOptions(wsId || ""));

  // Get current agent's runtime MCP servers
  const currentAgent = agentsData?.find((a) => a.id === assigneeId);
  const currentRuntime = runtimesData?.find((r) => r.id === currentAgent?.runtime_id);
  const agentMcpConfig = assigneeType === "agent" ? buildMcpConfigFromRuntime(currentRuntime) : null;

  // Handle assignee change: reset mcpConfig if it's no longer valid
  useEffect(() => {
    if (assigneeId !== previousAssigneeId) {
      setPreviousAssigneeId(assigneeId);
      
      // If assignee changed and we have an mcp selection, check if it's still valid
      if (mcpConfig && agentMcpConfig) {
        if (!isSelectionValidForAgent(mcpConfig, agentMcpConfig)) {
          setMcpConfig(null);
        }
      } else if (mcpConfig && !agentMcpConfig) {
        // If assignee is no longer an agent, clear mcp config
        setMcpConfig(null);
      }
    }
  }, [assigneeId, previousAssigneeId, mcpConfig, agentMcpConfig]);
  ...
  const updateMcpConfig = (config: string | null) => { setMcpConfig(config); };
        ...
        {backlogHintIssueId ? (
          ...
          <>
            ...
            <div className="flex items-center gap-1.5 px-4 py-2 shrink-0 flex-wrap">
              ...
              <McpPicker
                mcpConfig={mcpConfig}
                agentMcpConfig={agentMcpConfig ?? null}
                onUpdate={(u) => updateMcpConfig(u.mcp_config ?? null)}
                triggerRender={<PillButton />}
                align="start"
              />
            </div>
