...
import { projectListOptions, projectDetailOptions } from "@multica/core/projects";
...
function ProjectLink({ projectId }: { projectId: string }) {
  ...
  return (
    <AppLink href={wsPaths.projectDetail(projectId)} className="hover:underline">
      {project.title}
    </AppLink>
  );
}
...
function EditAutopilotDialog({
  ...
  autopilot: { id: string; title: string; description?: string | null; assignee_id: string; priority: string; execution_mode: string; issue_title_template?: string | null; project_id?: string | null };
  ...
  const [projectId, setProjectId] = useState(autopilot.project_id ?? "");
  ...
  useEffect(() => {
    ...
    setProjectId(autopilot.project_id ?? "");
  }, [autopilot]);

  const handleExecutionModeChange = (newMode: string | null) => {
    if (!newMode) return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 pt-2">
          ...
          {/* Project (conditional on execution mode) */}
              ...
              >
                ...
                <SelectContent>
                  ...
                  {projects.length === 0 && !projectsLoading ? (
                    <SelectItem value="__no_projects__" disabled>No projects available</SelectItem>
                  ) : (
                    projects.map((p) => (
                ...
                <div>
                  ...
                  <div className="mt-1">
                    {autopilot.project_id ? (
                      <ProjectLink projectId={autopilot.project_id} />
                    ) : (
