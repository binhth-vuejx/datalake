...
export function ChatWindow() {
  ...
  const containerClass = "fixed bottom-2 right-2 z-50 flex flex-col rounded-xl ring-1 ring-foreground/10 bg-sidebar shadow-2xl";
  const containerStyle: React.CSSProperties = {
    width: `${renderWidth}px`,
    height: `${renderHeight}px`,
    opacity: isVisible ? 1 : 0,
    // Avoid keeping `transform` on the element when fully visible — a
    // non-identity transform creates a new stacking context that clips
    // Radix UI portals (DropdownMenuContent) even when they render outside
    // this DOM subtree via document.body portal.
  ...
  return (
    <div ref={windowRef} className={containerClass} style={containerStyle}>
      ...
      {/*
        Inner wrapper clips visual content to the rounded border — but does NOT
        use CSS transform, so Radix UI portals (DropdownMenuContent) rendered
        outside this DOM subtree are not affected by the clip.
      */}
...
function AgentDropdown({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="max-h-80 w-auto max-w-64 z-[999999999]">
        {mine.length > 0 && (
          <DropdownMenuGroup>
