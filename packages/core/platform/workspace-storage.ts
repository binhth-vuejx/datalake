...
/** Current workspace slug (from URL). Falls back to last_workspace_slug cookie. */
export function getCurrentSlug(): string | null {
  if (_currentSlug) return _currentSlug;
  // Fallback: read from cookie set by the workspace layout on last visit
