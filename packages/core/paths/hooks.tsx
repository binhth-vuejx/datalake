...
/**
 * Like useWorkspacePaths but returns null instead of throwing when called
 * outside a workspace-scoped route (e.g. shared issue page).
 */
export function useWorkspacePathsSafe(): WorkspacePaths | null {
  const slug = useWorkspaceSlug();
  if (!slug) return null;
  return paths.workspace(slug);
}
