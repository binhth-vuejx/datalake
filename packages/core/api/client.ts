...
export class ApiClient {
  ...
  private async fetch<T>(path: string, init?: RequestInit & { skipAuth?: boolean }): Promise<T> {
    ...
    const headers: Record<string, string> = {
      ...
      "X-Request-ID": rid,
      ...(!init?.skipAuth ? this.authHeaders() : {}),
  ...
  async getIssueByShareToken(token: string): Promise<Issue> {
    return this.fetch(`/api/shared/issues/${token}`, {
      skipAuth: true,
    });
  }

  async generateIssueShareToken(id: string): Promise<{ share_token: string }> {
    return this.fetch(`/api/issues/${id}/share-token`, {
      method: "POST",
    });
  }
  ...
  async listSharedTimeline(shareToken: string): Promise<TimelineEntry[]> {
    return this.fetch(`/api/shared/issues/${shareToken}/timeline`, {
      skipAuth: true,
    });
  }
  ...
  async updateRuntimeMcpConfig(runtimeId: string, mcpConfig: string | null): Promise<AgentRuntime> {
    return this.fetch(`/api/runtimes/${runtimeId}/mcp-config`, {
      method: "PATCH",
  ...
  // Iframes
  async listIframes(): Promise<ListIframesResponse> {
    return this.fetch("/api/iframes");
  }

  async getIframe(id: string): Promise<Iframe> {
    return this.fetch(`/api/iframes/${id}`);
  }

  async createIframe(data: CreateIframeRequest): Promise<Iframe> {
    return this.fetch("/api/iframes", {
      method: "POST",
      body: JSON.stringify(data),
    });
  ...
  async updateIframe(id: string, data: UpdateIframeRequest): Promise<Iframe> {
    return this.fetch(`/api/iframes/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  async deleteIframe(id: string): Promise<void> {
    await this.fetch(`/api/iframes/${id}`, { method: "DELETE" });
  }
  ...
  // Sales
  async listSales(params?: { limit?: number; offset?: number }): Promise<ListSalesResponse> {
    const search = new URLSearchParams();
    if (params?.limit) search.set("limit", String(params.limit));
    if (params?.offset) search.set("offset", String(params.offset));
    return this.fetch(`/api/sales?${search}`);
  }
  ...
  async getSalesByCustomer(customerId: number, params?: { limit?: number; offset?: number }): Promise<ListSalesResponse> {
    const search = new URLSearchParams();
    if (params?.limit) search.set("limit", String(params.limit));
    if (params?.offset) search.set("offset", String(params.offset));
    return this.fetch(`/api/sales/customer/${customerId}?${search}`);
  }

  async createSale(data: CreateSaleRequest): Promise<CreateSaleResponse> {
    return this.fetch("/api/sales", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }

  async updateSale(id: number, data: UpdateSaleRequest): Promise<UpdateSaleResponse> {
    return this.fetch(`/api/sales/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  async deleteSale(id: number): Promise<DeleteSaleResponse> {
