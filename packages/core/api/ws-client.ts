...
export class WSClient {
  ...
  connect() {
    ...
    // Don't add workspace_slug to URL - Vite proxy doesn't forward query params for WebSocket
    // Instead, send it in the first message after connection opens

    this.ws = new WebSocket(url.toString());

    this.ws.onopen = () => {
      // Always send auth message with workspace_slug
      // For cookie auth: just send workspace_slug
      // For token auth: send both token and workspace_slug
      const payload = this.cookieAuth 
        ? { workspace_slug: this.workspaceSlug }
        : { token: this.token, workspace_slug: this.workspaceSlug };
