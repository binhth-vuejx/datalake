...
export const issueKeys = {
  ...
  shared: (token: string) => ["issues", "shared", token] as const,
};
...
export function sharedIssueTimelineOptions(shareToken: string) {
  return queryOptions({
    queryKey: ["shared", "timeline", shareToken],
    queryFn: () => api.listSharedTimeline(shareToken),
    enabled: !!shareToken,
  });
}
...
export function issueByShareTokenOptions(token: string) {
  return queryOptions({
    queryKey: issueKeys.shared(token),
    queryFn: () => api.getIssueByShareToken(token),
  });
}
