...
var PersistentWidget = (function() {
  ...
  var root_1 = /* @__PURE__ */ from_html(`<div class="panel svelte-1lqg09o"><div class="panel-header svelte-1lqg09o"><span class="title svelte-1lqg09o">🔍 Inspect</span> <button class="close-btn svelte-1lqg09o" aria-label="Close">✕</button></div> <button> </button> <pre class="info svelte-1lqg09o"> </pre> <!></div>`);
  var root = /* @__PURE__ */ from_html(`<div class="widget svelte-1lqg09o"><!> <button class="fab svelte-1lqg09o" aria-label="Toggle widget"> </button></div>`);
  const $$css = {
    hash: "svelte-1lqg09o",
    code: ".widget.svelte-1lqg09o {position:fixed;bottom:24px;right:24px;z-index:499;display:flex;flex-direction:column;align-items:flex-end;gap:8px;font-family:system-ui, sans-serif;}\n\n  /* ── Panel ── */.panel.svelte-1lqg09o {background:#1e1e2e;color:#cdd6f4;border:1px solid #45475a;border-radius:12px;padding:14px 16px;width:300px;box-shadow:0 8px 32px rgba(0, 0, 0, 0.5);font-size:13px;display:flex;flex-direction:column;gap:10px;}.panel-header.svelte-1lqg09o {display:flex;align-items:center;justify-content:space-between;}.title.svelte-1lqg09o {font-weight:600;font-size:14px;}.close-btn.svelte-1lqg09o {background:none;border:none;color:#6c7086;cursor:pointer;font-size:14px;padding:0;line-height:1;}.close-btn.svelte-1lqg09o:hover {color:#cdd6f4;}\n\n  /* ── Inspect button ── */.inspect-btn.svelte-1lqg09o {padding:6px 12px;border-radius:6px;border:none;font-size:12px;font-weight:600;cursor:pointer;background:#7c3aed;color:#fff;transition:background 0.15s;text-align:left;}.inspect-btn.active.svelte-1lqg09o {background:#dc2626;}.inspect-btn.svelte-1lqg09o:hover {filter:brightness(1.15);}\n\n  /* ── Info pre ── */.info.svelte-1lqg09o {margin:0;font-family:monospace;font-size:11px;color:#a6adc8;background:rgba(255, 255, 255, 0.04);border-radius:6px;padding:8px;white-space:pre-wrap;word-break:break-all;min-height:56px;line-height:1.6;}\n\n  /* ── Send button ── */.send-btn.svelte-1lqg09o {padding:6px 12px;border-radius:6px;border:none;font-size:12px;font-weight:600;cursor:pointer;background:#7c3aed;color:#fff;transition:background 0.15s;}.send-btn.sending.svelte-1lqg09o {background:#6c7086;cursor:default;}.send-btn.ok.svelte-1lqg09o      {background:#16a34a;cursor:default;}.send-btn.error.svelte-1lqg09o   {background:#dc2626;}.send-btn.svelte-1lqg09o:disabled {opacity:0.8;}\n\n  /* ── FAB ── */.fab.svelte-1lqg09o {width:48px;height:48px;border-radius:50%;background:#89b4fa;color:#1e1e2e;border:none;font-size:20px;cursor:pointer;box-shadow:0 4px 16px rgba(137, 180, 250, 0.4);transition:transform 0.15s ease, box-shadow 0.15s ease;display:flex;align-items:center;justify-content:center;}.fab.svelte-1lqg09o:hover {transform:scale(1.1);box-shadow:0 6px 20px rgba(137, 180, 250, 0.6);}"
  };
  function WidgetContent($$anchor, $$props) {
    push($$props, true);
    append_styles($$anchor, $$css);
    let isOpen = /* @__PURE__ */ state(false);
    let inspectActive = /* @__PURE__ */ state(false);
    let pinned = /* @__PURE__ */ state(false);
    let infoText = /* @__PURE__ */ state("Hover over an element to inspect it.");
    let pinnedEl = /* @__PURE__ */ state(null);
    let sendState = /* @__PURE__ */ state(
      "idle"
    ...
    function moveHighlight(el, isPinned) {
      ...
      highlight.style.background = isPinned ? "rgba(245,158,11,0.1)" : "rgba(124,58,237,0.1)";
    }
    ...
    function getVueFile(el) {
      function findVueInst(node2) {
        const keys = ["__vueParentComponent", "__vnode", "_vnode", "__vue_app__"];
        for (const k of keys) {
          if (node2[k]) {
            if (node2[k].component) return node2[k].component;
            if (node2[k].type) return node2[k];
          }
        }
        for (const k of Object.keys(node2)) {
          if (k.startsWith("__v") && node2[k]?.component) return node2[k].component;
          if (k.startsWith("__v") && node2[k]?.type?.__file) return node2[k];
        }
        return null;
      }
      let node = el;
    ...
    function toggleInspect() {
      set(inspectActive, !get(inspectActive));
      if (!get(inspectActive)) {
        set(pinned, false);
        set(pinnedEl, null);
        set(sendState, "idle");
        set(infoText, "Hover over an element to inspect it.");
        removeHighlight();
      }
    }
    async function sendIssue() {
      if (!get(pinnedEl)) return;
      set(sendState, "sending");
      try {
        const markdown = buildMarkdown(get(pinnedEl));
        const res = await fetch("http://localhost:8080/api/issues", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            title: `[Inspect] <${get(pinnedEl).tagName.toLowerCase()}> tại ${window.location.pathname}`,
    ...
    user_effect(() => {
      function onMouseOver(e) {
        if (!get(inspectActive) || get(pinned)) return;
        const el = e.target;
        if (!el || el === document.body || el === document.documentElement) return;
        if (el.closest?.("persistent-widget")) return;
        createHighlight();
        moveHighlight(el, false);
        set(infoText, getElementInfo(el), true);
      }
      function onMouseOut() {
        if (!get(inspectActive) || get(pinned)) return;
        if (highlight) {
    ...
    var node_1 = child(div);
    {
      var consequent_4 = ($$anchor2) => {
        ...
        let classes;
        var text$1 = child(button_1, true);
        reset(button_1);
        var pre = sibling(button_1, 2);
        var text_1 = child(pre, true);
        reset(pre);
        var node_2 = sibling(pre, 2);
        {
          var consequent_3 = ($$anchor3) => {
            ...
            var node_3 = child(button_2);
            {
              var consequent = ($$anchor4) => {
                var text_2 = text("📝 Tạo yêu cầu");
                append($$anchor4, text_2);
              };
              var consequent_1 = ($$anchor4) => {
                var text_3 = text("⏳ Đang gửi...");
                append($$anchor4, text_3);
              };
              var consequent_2 = ($$anchor4) => {
                var text_4 = text("✅ Đã tạo");
                append($$anchor4, text_4);
              };
              var alternate = ($$anchor4) => {
                var text_5 = text("❌ Lỗi — thử lại");
                append($$anchor4, text_5);
              };
              if_block(node_3, ($$render) => {
                if (get(sendState) === "idle") $$render(consequent);
                else if (get(sendState) === "sending") $$render(consequent_1, 1);
                else if (get(sendState) === "ok") $$render(consequent_2, 2);
                else $$render(alternate, -1);
              });
            }
            reset(button_2);
            template_effect(() => {
              classes_1 = set_class(button_2, 1, "send-btn svelte-1lqg09o", null, classes_1, {
                sending: get(sendState) === "sending",
                ok: get(sendState) === "ok",
                error: get(sendState) === "error"
              });
              button_2.disabled = get(sendState) === "sending" || get(sendState) === "ok";
            });
            delegated("click", button_2, sendIssue);
        ...
        reset(div_1);
        template_effect(() => {
          classes = set_class(button_1, 1, "inspect-btn svelte-1lqg09o", null, classes, { active: get(inspectActive) });
          set_text(text$1, get(inspectActive) ? "✋ Stop Inspect" : "🔍 Start Inspect");
          set_text(text_1, get(infoText));
        });
        delegated("click", button, () => set(isOpen, false));
        delegated("click", button_1, toggleInspect);
        append($$anchor2, div_1);
      };
      if_block(node_1, ($$render) => {
        if (get(isOpen)) $$render(consequent_4);
      });
    }
    ...
    reset(button_3);
    reset(div);
    template_effect(() => set_text(text_6, get(isOpen) ? "✕" : "�"));
    delegated("click", button_3, () => set(isOpen, !get(isOpen)));
    append($$anchor, div);
    pop();
  }
  delegate(["click"]);
  customElements.define("persistent-widget", create_custom_element(WidgetContent, {}, [], [], { mode: "open" }));
